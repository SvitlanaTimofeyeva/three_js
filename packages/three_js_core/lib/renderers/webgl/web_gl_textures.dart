...
class WebGLTextures {
  ...
  final bool supportsInvalidateFramebuffer =
      false; //typeof navigator === 'undefined' ? false : /OculusBrowser/g.test( navigator.userAgent );
  ...
  int getInternalFormat(
    ...
    int glType,
    String colorSpace, [
    bool forceLinearTransfer = false,
  ]) {
    if (internalFormatName != null) {
      if (WebGL.get(internalFormatName) != null)
        return WebGL.get(internalFormatName)!;
      console.warning(
    ...
    if (glFormat == WebGL.RGB) {
      if (glType == WebGL.UNSIGNED_INT_5_9_9_9_REV)
        internalFormat = WebGL.RGB9_E5;
    }

    if (glFormat == WebGL.RGBA) {
      final String transfer = forceLinearTransfer
          ? LinearTransfer
          : ColorManagement.getTransfer(ColorSpace.fromString(colorSpace));
      ...
      if (glType == WebGL.UNSIGNED_SHORT_5_5_5_1)
        internalFormat = WebGL.RGB5_A1;
    }
  ...
  int getMipLevels(Texture texture, image) {
    if (textureNeedsGenerateMipmaps(texture) ||
      ...
      return MathUtils.log2(math.max(image.width, image.height)).toInt() + 1;
    ...
    if (renderTarget is WebGLCubeRenderTarget) {
      ...
      if (renderTargetProperties["__webglColorRenderbuffer"] != null) {
        for (
          ...
          if (renderTargetProperties['__webglColorRenderbuffer'][i] != null)
  ...
  void setTexture2D(Texture texture, int slot) {
    ...
    if (!texture.isRenderTargetTexture &&
      ...
      if (image == null) {
        console.warning(
          'WebGLRenderer: Texture marked for update but image is null',
        );
  ...
  void setTextureParameters(textureType, Texture texture, [supportsMips]) {
    ...
    if (textureType == WebGL.TEXTURE_3D ||
        textureType == WebGL.TEXTURE_2D_ARRAY) {
      _gl.texParameteri(
        textureType,
    ...
    if (texture is DepthTexture && texture.compareFunction != null) {
      _gl.texParameteri(
        ...
        WebGL.COMPARE_REF_TO_TEXTURE,
      );
      _gl.texParameteri(
        textureType,
        WebGL.TEXTURE_COMPARE_FUNC,
    ...
    if (extensions.has('EXT_texture_filter_anisotropic')) {
      ...
      if (texture.minFilter != NearestMipmapLinearFilter &&
          texture.minFilter != LinearMipmapLinearFilter)
        return;
      if (texture.type == FloatType &&
          !extensions.has('OES_texture_float_linear'))
      ...
      if (texture.anisotropy > 1 ||
        ...
        if (kIsWeb && !kIsWasm) {
          ...
          gl.texParameterf(
            textureType,
            WebGL.TEXTURE_MAX_ANISOTROPY_EXT,
            math
                .min(texture.anisotropy, capabilities.getMaxAnisotropy())
                .toDouble(),
          );
  ...
  void updateTexture(
    ...
    if (updateRanges.length == 0) {
      ...
      final currentUnpackSkipPixels = _gl.getParameter(
        WebGL.UNPACK_SKIP_PIXELS,
      );
  ...
  void uploadTexture(
    ...
    if (source.version != source.currentVersion || forceUpload) {
      ...
      if (kIsWeb) {
        ...
        _gl.pixelStorei(
          WebGL.UNPACK_PREMULTIPLY_ALPHA_WEBGL,
              ...
              if (glFormat > 0) {
                if (useTexStorage && dataReady) {
                  if (texture.layerUpdates.isNotEmpty) {
                    final layerByteLength = TextureUtils.getByteLength(
                      mipmap.width,
                      mipmap.height,
                      texture.format,
                      texture.type,
  ...
  void uploadCubeTexture(
    ...
    if (source.version != source.currentVersion || forceUpload) {
      ...
      _gl.pixelStorei(WebGL.UNPACK_ALIGNMENT, texture.unpackAlignment);
      if (kIsWeb) {
        final workingPrimaries = ColorManagement.getPrimaries(
          ColorManagement.workingColorSpace,
        );
        final texturePrimaries = texture.colorSpace == NoColorSpace
            ? null
            : ColorManagement.getPrimaries(
                ColorSpace.fromString(texture.colorSpace),
              );
        final unpackConversion =
            texture.colorSpace == NoColorSpace ||
                workingPrimaries == texturePrimaries
            ? WebGL.NONE
            : WebGL.BROWSER_DEFAULT_WEBGL;
        ...
        _gl.pixelStorei(
          WebGL.UNPACK_COLORSPACE_CONVERSION_WEBGL,
          unpackConversion,
        );
      }

      final isCompressed =
          (texture.isCompressedTexture || texture is CompressedTexture);
      final isDataTexture =
          (texture.image[0] != null && texture is DataTexture);
      final isCubeTexture =
          (texture.image[0] != null && texture is CubeTexture);
      print('here1');
      final cubeImage = [];
          ...
          for (int j = 0; j < mipmaps.length; j++) {
            ...
            if (texture.format != RGBAFormat) {
              if (glFormat != null) {
                if (useTexStorage && dataReady) {
                  state.compressedTexSubImage2D(
                    WebGL.TEXTURE_CUBE_MAP_POSITIVE_X + i,
                    j,
                    0,
                    0,
                    mipmap.width,
  ...
  void setupFrameBufferTexture(
    ...
    final glInternalFormat = getInternalFormat(
      texture.internalFormat,
      glFormat,
      glType,
      texture.colorSpace,
    ...
    if (renderTargetProperties['__hasExternalTextures'] == null) {
      ...
      if (textureTarget == WebGL.TEXTURE_3D ||
          textureTarget == WebGL.TEXTURE_2D_ARRAY) {
        state.texImage3D(
          textureTarget,
          level,
          glInternalFormat,
    ...
    } else if (textureTarget == WebGL.TEXTURE_2D ||
        (textureTarget >= WebGL.TEXTURE_CUBE_MAP_POSITIVE_X &&
            textureTarget <= WebGL.TEXTURE_CUBE_MAP_NEGATIVE_Z)) {
      // see #24753
  ...
  void setupRenderBufferStorage(
    ...
    if (renderTarget.depthBuffer) {
      ...
      final depthType = depthTexture != null && depthTexture.isDepthTexture
          ? depthTexture.type
          : null;
      final glInternalFormat = getInternalDepthFormat(
        renderTarget.stencilBuffer,
        depthType,
      );
      final glAttachmentType = renderTarget.stencilBuffer
          ? WebGL.DEPTH_STENCIL_ATTACHMENT
  ...
  void setupDepthTexture(framebuffer, RenderTarget renderTarget) {
    ...
    if (renderTargetProperties['__boundDepthTexture'] !=
    ...
    if (renderTarget.depthTexture != null &&
      ...
      if (isCube) {
        ...
        if (renderTargetProperties['__webglDepthbuffer'] == null) {
          renderTargetProperties['__webglDepthbuffer'] = _gl
              .createRenderbuffer();
          setupRenderBufferStorage(
            renderTargetProperties['__webglDepthbuffer'],
            renderTarget,
    ...
    if (renderTarget.depthTexture != null &&
      ...
      if (isCube) {
        ...
        for (int i = 0; i < 6; i++) {
          ...
          if (renderTargetProperties['__webglDepthbuffer'].length <= i ||
              renderTargetProperties['__webglDepthbuffer'][i] == null) {
            (renderTargetProperties['__webglDepthbuffer'] as List).listSetter(
              i,
    ...
    if (isCube) {
      ...
      for (int i = 0; i < 6; i++) {
        if (texture.mipmaps.isNotEmpty) {
          ...
          (renderTargetProperties['__webglFramebuffer'] as List).listSetter(
            i,
      ...
      if ((renderTarget.samples > 0) &&
        ...
        for (int i = 0; i < textures.length; i++) {
          final texture = textures[i];
          (renderTargetProperties['__webglColorRenderbuffer'] as List)
              .listSetter(i, _gl.createRenderbuffer());
  ...
  void updateRenderTargetMipmap(RenderTarget renderTarget) {
    ...
    for (int i = 0, il = textures.length; i < il; i++) {
      ...
      if (textureNeedsGenerateMipmaps(texture)) {
        final target = renderTarget is WebGLCubeRenderTarget
            ? WebGL.TEXTURE_CUBE_MAP
            : WebGL.TEXTURE_2D;
  ...
  void updateMultisampleRenderTarget(RenderTarget renderTarget) {
    if (renderTarget.samples > 0) {
      if (!useMultisampledRTT(renderTarget)) {
        ...
        final depthStyle = renderTarget.stencilBuffer
            ? WebGL.DEPTH_STENCIL_ATTACHMENT
            : WebGL.DEPTH_ATTACHMENT;
        final renderTargetProperties = properties.get(renderTarget);
        final isMultipleRenderTargets = (textures.length > 1);
  ...
  verifyColorSpace(Texture texture, image) {
    ...
    if (colorSpace != LinearSRGBColorSpace && colorSpace != NoColorSpace) {
      ...
      if (ColorManagement.getTransfer(ColorSpace.fromString(colorSpace)) ==
          SRGBTransfer) {
