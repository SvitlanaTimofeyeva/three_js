...
class WebGLRenderer {
  ...
  Map<String, int> get uniformsGroupsStats => {
    'buffers': uniformsGroups.buffers.length,
    'updateList': uniformsGroups.updateList.length,
    'allocatedBindingPoints': uniformsGroups.allocatedBindingPoints.length,
    'maxBindingPoints': uniformsGroups.maxBindingPoints,
  };

  void debugPrintUniformsGroupsStats() {
    uniformsGroups.debugPrintStats();
  }
  ...
  void render(Object3D scene, Camera camera) {
    ...
    // 🔧 After a full top-level render, clean up per-camera transmission RTs.
    // We assume the current camera is the only live one here.
    if (renderStateStack.isEmpty) {
      renderStates.pruneTransmissionTargets({camera.id}.toSet());
    }
  }
            ...
            if (groups.isNotEmpty) {
              for (int i = 0, l = groups.length; i < l; i++) {
                ...
                if (group["materialIndex"] < material.children.length) {
                  final groupMaterial =
                      material.children[group["materialIndex"]];
  ...
  void renderTransmissionPass(
    ...
    final RenderTarget transmissionRenderTarget =
        currentRenderState?.state.transmissionRenderTarget[camera.id];
